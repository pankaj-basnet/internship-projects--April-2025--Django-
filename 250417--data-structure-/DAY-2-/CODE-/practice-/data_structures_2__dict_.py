...
def display_name():
    ...
    # for (item_name, quantity ) in sorted(shop_items): # ValueError: too many values to unpack (expected 2)
...
def add_new_product_to_inventory ( item_name, number_of_items):
    print(f"addition's processing ... ({item_name} --> {number_of_items})")
...
# add_new_product_to_inventory("coffee", 100) # ValueError
add_new_product_to_inventory("tea", 60)
    

def restock_item_to_inventory ( item_name, number_of_items):
    print(f"restock's processing ... ({item_name} --> {number_of_items})")
...
def sell_item_to_inventory ( item_name, number_of_items):
    print(f"sale's processing ... ({item_name} --> {number_of_items})")
