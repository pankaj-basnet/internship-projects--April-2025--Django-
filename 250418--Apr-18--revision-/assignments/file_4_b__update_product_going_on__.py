...
def update_product(name = "sugar222", quantity = 99):
    ...
    try:
        with open(csv_filename, 'r', newline='') as ctx:

            dict_reader = csv.DictReader(ctx)
            # next(ctx)

            product_list = list(dict_reader)

            ### update product_list in memory while 'r' (reading csv , not in 'w'(writing csv))
        ...
        ## print after with open statements -- reading and writing done 'w' and 'r'  {sn=}
        print(f"✅ product quantity {quantity} updated ( for: {name} )")
